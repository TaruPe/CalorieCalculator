...
export default function App() {
  // State variables weight, intensity, gender and calculated calorie count
  const [weight, setWeight] = useState(0);
  const [intensity, setIntensity] = useState(1.3);
  const [gender, setGender] = useState('male');
  const [calories, setCalories] = useState(0);

  // Selection options for intensities and genders
  ...
  // Function for counting calories
  const calculate = () => {
    let result = 0;
    if (gender === 'male') {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.field}>
        <TextInput
          ...
          placeholder='in kilograms' // Empty input field
  ...
  <RadioForm
    style={styles.radio}
    buttonSize={10}
    radio_props={genders} // Selection options
    initial={0} // Initialized selection
